...
import at.ac.tuwien.dsg.sybl.controlService.staticData.ActionEffects;
import at.ac.tuwien.dsg.sybl.controlService.utils.PlanningLogger;
import at.ac.tuwien.dsg.sybl.monitorandenforcement.runtimeapi.MappingFromWS;
...
public class PlanningGreedyAlgorithm implements Runnable {
	...
	public void findBestActions() {
		HashMap<String, List<ActionEffect>> actionEffects = ActionEffects
				.getActionEffects(dependencyGraph,syblAPI);
			...
			if (maxAction > 0) {
				...
				if (fixedConstraints.get(action) != 0) {
					if (action.getActionType().equalsIgnoreCase("scaleout")) {
						at.ac.tuwien.dsg.sybl.monitorandenforcement.runtimeapi.Node translatedEntity = MappingToWS.mapNodeToNode(entity);
				
						syblAPI.scaleout(translatedEntity);
						...
						if (action.getActionType().equalsIgnoreCase("scalein")) {
							at.ac.tuwien.dsg.sybl.monitorandenforcement.runtimeapi.Node translatedEntity = MappingToWS.mapNodeToNode(entity);
							syblAPI.scalein(translatedEntity);
							try {
								Thread.sleep(120000);
							} catch (InterruptedException e) {
								// TODO Auto-generated catch block
...
//						}
		...
		for (Pair<ActionEffect, Integer> actionEffect : result)
			...
			//	for (int i = 0; i < actionEffect.getSecond(); i++) {
					syblAPI.scaleout(MappingToWS.mapNodeToNode(actionEffect.getFirst()
							.getTargetedEntity()));
					try {
						Thread.sleep(60000);
					} catch (InterruptedException e) {
...
//					for (int i = 0; i < actionEffect.getSecond(); i++) {
						...
						syblAPI.scalein(MappingToWS.mapNodeToNode(actionEffect.getFirst()
								.getTargetedEntity()));
						try {
							Thread.sleep(60000);
						} catch (InterruptedException e) {
	...
	public void run() {
		while (true) {
			try {
				...
				Node cloudService = MappingFromWS.mapNodeToNode(syblAPI.getControlledService(), new ArrayList<Node>());			
