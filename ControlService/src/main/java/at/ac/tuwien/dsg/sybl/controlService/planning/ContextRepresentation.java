...
*/
package at.ac.tuwien.dsg.sybl.controlService.planning;
...
import at.ac.tuwien.dsg.sybl.controlService.staticData.ActionEffect;
...
public class ContextRepresentation {
	...
	public ContextRepresentation(DependencyGraph cloudService, SyblAPIService syblAPI){
	   			...
	   			for (Monitoring monitoring:syblSpecification.getMonitoring()){
					...
					if (!methodName.equals("")) {
							try {
								...
								parameters[0]=MappingToWS.mapNodeToNode(entity);
								partypes[0]=at.ac.tuwien.dsg.sybl.monitorandenforcement.runtimeapi.Node.class;
								
								Method method = SyblAPIService.class.getMethod(methodName,partypes);

								value = (Float)method.invoke(syblAPI,parameters);
							}catch(Exception e){
								e.printStackTrace();
							}
						}else{
							
						 value= (Float) syblAPI.getMetricValue(monitoring.getMonitor().getMetric(), MappingToWS.mapNodeToNode(entity));
		   			...
		   			for (BinaryRestriction restriction:constraint.getToEnforce().getBinaryRestriction()){
		   				...
		   				if (!methodName.equals("")) {
							try {
							 ...
							 value= (Float) syblAPI.getMetricValue(metric, MappingToWS.mapNodeToNode(entity));
